...
uses
  ...
  ImgList, ShlObj, ShellAPI, ActiveX, ComObj, Registry, Menus,
  {$IFDEF COMPILER_6_UP}
  Variants,
  {$ENDIF}
  IDEVirtualResources, IDEVirtualUtilities, IDEVirtualShellTypes,
  {$IFDEF VIRTUALNAMESPACES} IDEVirtualNamespace, {$ENDIF}
  IDEVirtualPIDLTools, IDEVirtualUnicodeDefines;
...
{-------------------------------------------------------------------------------}
  ...
  TEnumFolderCallback = function(APIDL: PItemIDList; AParent: TNamespace;
...
{-------------------------------------------------------------------------------}
  ...
  protected
    ...
    function InjectCustomSubMenu(Menu: HMenu; Caption: string; PopupMenu: TPopupMenu;
  ...
  public
    ...
    function Drop(const dataObj: IDataObject; grfKeyState: Integer;
      pt: TPoint; var dwEffect: Integer): HResult;
    function EnumerateFolder(Folders, NonFolders, IncludeHidden: Boolean;
    ...
    procedure InvalidateRelativePIDL(Folders, NonFolders, IncludeHidden: Boolean);
    ...
    function Paste(NamespaceArray: TNamespaceArray): Boolean;
    ...
    function SubFoldersEx(Flags: Longword): Boolean;
    function SubItemsEx(Flags: Longword): Boolean;
...
begin
  ...
  if FileExample <> '' then
  begin
    SHGetFileInfo(PChar(FileExample), Attrib, FileInfo, SizeOf(TSHFileInfo), Flags);
...
begin
  ...
  begin
    ...
    begin
      ...
      Found :=  CurrentContextMenu.GetCommandString(MenuItemID-1, GCS_HELPTEXTW, nil, Pointer(Result),
        BufferLen) = NOERROR
    end;
    if not Found then
    begin
...
begin
  ...
  begin
    ...
    begin
      SetLength(Result, BufferLen);
      Found :=  CurrentContextMenu.GetCommandString(MenuItemID-1, GCS_VERBW, nil, Pointer(Result),
        BufferLen) = NOERROR
    end;
    if not Found then
    begin
...
function TNamespace.EnumerateFolder(Folders, NonFolders,
...
begin
  ...
  try
   ...
   //     Flags := Flags or SHCONTF_INIT_ON_FIRST_NEXT or SHCONTF_NETPRINTERSRCH or
   //        SHCONTF_SHAREABLE or SHCONTF_STORAGE;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            InvokeInfo.lpVerb := PAnsiChar(AnsiString(AVerbA));
        end;
...
begin
  ...
  begin
    { The Shell only caches the LastWrite time in the PIDL }
    GetFileTimes;
    if IsUnicode then
    begin
      if Assigned(FWin32FindDataW) and FileSystem then
...
begin
  Result := ExtractFileExtW(NameParseAddress)
end;
...
begin
  ...
  begin
    if not Assigned(FWin32FindDataA) then
      GetDataFromIDList;
    if Assigned(FWin32FindDataA) and FileSystem then
      Result := WideString(FWin32FindDataA^.cFileName)
    else
      Result := '';
  end
end;
...
function TNamespace.SubFoldersEx(Flags: Longword): Boolean;
begin
  Result := InternalSubItems(Flags)
end;

function TNamespace.SubItemsEx(Flags: Longword): Boolean;
begin
  Result := InternalSubItems(Flags)
end;
...
begin
  ...
  begin
    ...
    Drive := AnsiString(DriveW);
    if DirExistsW(DriveW) then
    begin
      if IsUnicode then
      ...
      if not IsWin95_SR1 then
        ValidData := GetDiskFreeSpaceFAT32(
          PChar( {$IFDEF UNICODE}DriveW{$ELSE}Drive{$ENDIF}), SectorsPerCluster, BytesPerSector, FreeClusters, TotalClusters)
      else
        ValidData := GetDiskFreeSpaceA(
          PAnsiChar( Drive), SectorsPerCluster, BytesPerSector, FreeClusters, TotalClusters);
...
begin
  ...
  begin
    if not Assigned(FWin32FindDataW) then
      GetDataFromIDList;
    if Assigned(FWin32FindDataW) and FileSystem then
      with FWin32FindDataW^ do
        Result := Int64(nFileSizeLow) + nFileSizeHigh shl 32
    else
      Result := 0
  end else
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FSHGetFileInfoRec^.FileType := WideString(InfoA.szTypeName);
        { NT only half-assed supports the SHGetFileInfo...only if the ext is      }
        { associated with a program. So we build it ourselves                     }
        if FSHGetFileInfoRec^.FileType = '' then
          FSHGetFileInfoRec^.FileType := AnsiUpperCase(ExtractFileExtW(NameForParsing)) + STR_FILE;
...
begin
  ...
  begin
    ...
    if Assigned(FWin32FindDataA) and FileSystem then
    begin
      Result := WideString(FWin32FindDataA^.cAlternateFileName);
      if Result = '' then
        Result := AnsiUpperCase(string(FWin32FindDataA^.CFileName))
    end else
      Result := '';
  end
end;
...
procedure TNamespace.InvalidateRelativePIDL(Folders, NonFolders, IncludeHidden: Boolean);
var
  Enum: IEnumIDList;
  Flags: Longword;
  Fetched: Longword;
...
function TNamespace.Paste(NamespaceArray: TNamespaceArray): Boolean;
var
  NSA: TNamespaceArray;
  i: integer;
begin
  ...
  begin
    ...
    begin
      ...
      Result := PhysicalDesktopFolder.Paste(NSA);
    end else
    begin
      if VerifyPIDLRelationship(NamespaceArray) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Success := Succeeded(ShellLinkAInterface.GetPath(PAnsiChar(AnsiString(S)), MAX_PATH, FindData, SLGP_UNCPRIORITY));
        if Success then
        begin
        ...
        SetLength(S, BUFFERSIZE);
        Success := Succeeded(ShellLinkAInterface.GetArguments(PAnsiChar(AnsiString(S)), BUFFERSIZE));
        if Success then
        begin
          SetLength(S, lstrlen(PChar( S)));
          FArguments := S
        ...
        SetLength(S, BUFFERSIZE);
        Success := Succeeded(ShellLinkAInterface.GetDescription(PAnsiChar(AnsiString(S)), BUFFERSIZE));
        if Success then
        begin
          SetLength(S, lstrlen(PChar( S)));
          FDescription := S
        ...
        SetLength(S, BUFFERSIZE);
        Success := Succeeded(ShellLinkAInterface.GetWorkingDirectory(PAnsiChar(AnsiString(S)), BUFFERSIZE));
        if Success then
        begin
          SetLength(S, lstrlen(PChar( S)));
          FWorkingDirectory := S
        ...
        SetLength(S, BUFFERSIZE);
        Success := Succeeded(ShellLinkAInterface.GetIconLocation(PAnsiChar(AnsiString(S)), BUFFERSIZE, FIconIndex));
        if Success then
        begin
          SetLength(S, lstrlen(PChar( S)));
          FIconLocation := S
  ...
  if (TargetPath = '') and not Assigned(TargetIDList) and not SilentWrite then
    ...
    begin
      ...
      begin
        FFileName := LinkFileName;
        S := FTargetPath;
        ShellLinkAInterface.SetPath(PAnsiChar(AnsiString(S)));
        S := FArguments;
        ShellLinkAInterface.SetArguments(PAnsiChar(AnsiString(S)));
        S := FDescription;
        ShellLinkAInterface.SetDescription(PAnsiChar(AnsiString(S)));
        S := FTargetPath;
        ShellLinkAInterface.SetPath(PAnsiChar(AnsiString(S)));
        S := FWorkingDirectory;
        ShellLinkAInterface.SetWorkingDirectory(PAnsiChar(AnsiString(S)));
        S := FIconLocation;
        ShellLinkAInterface.SetIconLocation(PAnsiChar(AnsiString(S)), FIconIndex);

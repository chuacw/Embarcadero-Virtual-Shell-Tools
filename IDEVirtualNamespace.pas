...
uses
  ...
  ShellAPI, ActiveX, IDEVirtualPIDLTools, IDEVirtualUtilities, IDEVirtualDataObject,
  IDEVirtualShellTypes, IDEVirtualWideStrings, IDEVirtualShellContainers, IDEVirtualShellNotifier;
...
begin
  ...
  s := Text;
  InsertMenu(Menu, 0, MF_BYPOSITION or MF_STRING, CommandID, PChar(s));
end;
...
begin
  ...
  begin
    if HiWord( Longword(WideString(InvokeInfo.lpVerb))) = 0 then
    begin
      Result := FindMenuItemByVerb(WideString(InvokeInfo.lpVerb));
      InvokedByVerb := True
    end else
...
function TVirtualIContextMenu.HandleMenuMsg2(uMsg: UINT; wParam,
  lParam: Integer; var lpResult: LRESULT): HResult;
begin
  // Needs to be implemented
  Result := E_NOTIMPL
end;
...
// namespace. Return what occurred in the DropEffecct parameter.
...
// many times so don't do a lot of processing here.  Return what occurred in the

...
uses
  ...
  ActiveX, ShlObj, ShellAPI, AxCtrls, IDEVirtualPIDLTools, IDEVirtualShellTypes,
...
begin
  ...
  begin
    ...
    Inc(PByte(Head),  SizeOf(FCIDA.cidl) + (SizeOf(FCIDA.aoffset) * APIDLList.Count));
...
begin
  if Assigned(FCIDA) and (Index > -1) and (Index < PIDLCount) then
    Result := PItemIDList( PByte(FCIDA) + PDWORD(PByte(@FCIDA^.aoffset)+sizeof(FCIDA^.aoffset[0])*(1+Index))^)
  else
    Result := nil
end;
...
begin
  ...
  if Assigned(DropFiles) then
  begin
    Head := PByte( DropFiles) + DropFiles^.pFiles;
    Len := 2 * (lstrlenW(PWideChar( Head)));
    while Len > 0 do
    begin
      Head := Head + Len + 2;
...
begin
  FillChar(Result, SizeOf(Result), #0);
  StrCopy(Result.cFileName, PAnsiChar(AnsiString(FileName)));
end;

unit IDEVirtualUnicodeControls;
...
uses
  ...
  StdCtrls, ActiveX, ComCtrls, Commctrl, ImgList, IDEVirtualWideStrings,
  IDEVirtualShellUtilities, IDEVirtualUtilities;
...
uses Imm, IDEVirtualShellContainers;
...
procedure InitControls;
  ...
  begin
    WindowAtomString := AnsiString(Format('Delphi%.8X',[GetCurrentProcessID]));
    ControlAtomString := AnsiString(Format('ControlOfs%.8X%.8X', [HInstance, GetCurrentThreadID]));
  end;
  ...
  {$IFDEF VER190}
  procedure InitAtomStrings; // Delphi 2007
  begin
    InitAtomStrings_D5_D6_D7;
  end;
  {$ENDIF}
  ...
  {$IFDEF VER200}
  procedure InitAtomStrings; // Delphi 2009
  begin
    InitAtomStrings_D5_D6_D7;
  end;
  {$ENDIF}
  ...
  {$IFDEF VER210}
  procedure InitAtomStrings; // Weaver
  begin
    InitAtomStrings_D5_D6_D7;
  end;
  {$ENDIF}
...
begin
  InitAtomStrings;
  WindowAtom := GlobalAddAtomA(PAnsiChar(WindowAtomString));
  ControlAtom := GlobalAddAtomA(PAnsiChar(ControlAtomString));
end;
...
begin
  ...
  and (GetWindowLongW(HWindow, GWL_ID) = 0) then
    SetWindowLongW(HWindow, GWL_ID, Longint(HWindow));  //added Longint cast to prevent range check error on Windows 7
...
begin
    ...
    SubClassUnicodeControl(Control, PAnsiChar(AnsiString(Params.Caption)), IDEWindow);
  end;
end;
function TntControl_IsCaptionStored(Control: TControl): Boolean;
begin
...
begin
  ...
  or ((Control is TWinControl) and TWinControl(Control).HandleAllocated and (not IsWindowUnicode(TWinControl(Control).Handle))) then
    ...
    if WideCaptionHolder = nil then
      ...
      Result := GetSyncedWideString(WideCaptionHolder.FWideCaption, AnsiString(TAccessControl(Control).Text));
...
begin
  ...
  or ((Control is TWinControl) and TWinControl(Control).HandleAllocated and (not IsWindowUnicode(TWinControl(Control).Handle))) then
    ...
    with FindWideCaptionHolder(Control) do
      SetSyncedWideString(Text, FWideCaption, AnsiString(TAccessControl(Control).Text), SetAnsiText)
  end else if (not TWinControl(Control).HandleAllocated) then begin
    // NO HANDLE
...
begin
  if AnsiString(WideStr) <> (AnsiStr) then begin
    WideStr := WideString(AnsiStr); {AnsiStr changed.  Keep WideStr in sync.}
  end;
  Result := WideStr;
end;
...
begin
  ...
  begin
    if (not WideSameStr(Value, WideString(AnsiString(Value)))) {unicode chars lost in conversion}
    and (AnsiStr = AnsiString(Value))  {AnsiStr is not going to change}
    then begin
      SetAnsiStr(''); {force the change}
    end;
    WideStr := Value;
    SetAnsiStr(AnsiString(Value));
  end;
end;
...
begin
  ...
  TntControl_SetText(FControl, TntControl_GetStoredText(FControl, WideString(Params_Caption)));
end;
...
begin
  TAccessControl(FControl).Text := string(Value);
end;
...
initialization
  ...
  WideCaptionHolders := TObjectList.Create(True);

unit IDEVirtualSystemImageLists;
...
function VirtualSysImages.GetIconIndex(const Filename: string): Integer;
var
  shfi: SHFILEINFO;
  Flags: Longword;
begin
  ...
  if (Length(Filename) > 0) and (SHGetFileInfo(PWideChar(Filename),
    FILE_ATTRIBUTE_ARCHIVE or FILE_ATTRIBUTE_NORMAL,
    shfi, SizeOf(SHFILEINFO), Flags) <> 0) then
      Result := shfi.iIcon;
end;

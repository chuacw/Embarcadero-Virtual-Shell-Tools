unit IDEVirtualShellAutoComplete;
...
uses
  ...
  ImgList, ShlObj, ShellAPI, ActiveX, ComObj, IDEVirtualShellTypes,
  IDEVirtualWideStrings, IDEVirtualShellUtilities, IDEVirtualShellContainers;
...
type
  ...
  private
    ...
    function GetStringsW: TWideStringList;
  ...
  protected
    function EnumFolder(APIDL: PItemIDList; AParent: TNamespace; Data: Pointer; var Terminate: Boolean): Boolean;
    ...
    property StringsW: TWideStringList read GetStringsW;
  public
...
begin
  ...
  FStringsW := TWideStringList.Create;
...
function TCustomVirtualShellAutoComplete.EnumFolder(APIDL: PItemIDList;
...
begin
  ...
  begin
    ...
    begin
      NS.EnumerateFolder(accFileSysDirs in FContents, accFileSysFiles in FContents,
...
var
  NS: TNamespace;
begin
  if (accCurrentDir in FContents) and DirExistsW(FCurrentDir) then
  begin
    try
...
begin
  if (accDesktop in FContents) and Assigned(DesktopFolder) then
    DesktopFolder.EnumerateFolder(accFileSysDirs in FContents, accFileSysFiles in FContents,
      accHidden in FContents, EnumFolder, nil);
end;
...
begin
  if (accFavorites in FContents) and Assigned(FavoritesFolder) then
    FavoritesFolder.EnumerateFolder(accFileSysDirs in FContents, accFileSysFiles in FContents,
      accHidden in FContents, EnumFolder, nil);
end;
...
begin
  if (accMyComputer in FContents) and Assigned(DrivesFolder) then
    DrivesFolder.EnumerateFolder(accFileSysDirs in FContents, accFileSysFiles in FContents,
      accHidden in FContents, EnumFolder, nil);
end;
...
function TCustomVirtualShellAutoComplete.GetStringsW: TWideStringList;
 var
  i: integer;
begin
  FStringsW.Clear;
  for i := 0 to Namespaces.Count - 1 do
    FStringsW.Add(Namespaces[i].NameParseAddress);
  if accSortList in FContents then
    FStringsW.Sort;
  Result := FStringsW
end;
...
begin
  if (Value <> FCurrentDir) and DirExistsW(Value) then
  begin
    Dirty := True;
    FCurrentDir := Value;
  end

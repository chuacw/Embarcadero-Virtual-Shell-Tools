...
uses
  Windows, Messages, Classes, IDEVirtualShellTypes,
...
begin
  inherited;
  HelperWnd := {$IFNDEF COMPILER_6_UP}Forms{$else}Classes{$ENDIF}.AllocateHWnd(HelperWndProc);
end;
...
begin
  ...
  {$IFNDEF COMPILER_6_UP}Forms{$else}Classes{$ENDIF}.DeAllocateHWnd(HelperWnd);
  inherited;
end;
...
var
  ...
  sFileName, sInitialDirectory, sParameters: string;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        StartupInfo.wShowWindow := SW_HIDE;    // Don't show the service on the screen
        ...
        StartupInfo.hStdOutput := PipeChildOut;
        StartupInfo.hStdInput := PipeChildIn;
        StartupInfo.hStdError := PipeChildErrorOut;
        StartupInfo.dwFlags := STARTF_USESHOWWINDOW or STARTF_USESTDHANDLES;

        sInitialDirectory := InitialDirectory;
        if sInitialDirectory = '' then
          szDirectory := nil
        else
          szDirectory := PChar(sInitialDirectory);
        ...
        if sFileName = '' then
          ...
          TempCharFile := PChar(sFileName);

        sParameters := Parameters;
        if sParameters = '' then
          TempCharParams := nil
        else
          TempCharParams := PChar(sParameters);
        ...
        if not CreateProcess(TempCharFile, TempCharParams, nil, nil, True, 0,
          nil, szDirectory, StartupInfo, FProcessInfo)
        then
